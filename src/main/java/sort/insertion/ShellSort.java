...
public class ShellSort<T> implements ISort<T> {
  ...
  public T[] sort(T[] data, Comparator<T> comparator, Increment increment) {
    ...
    for (int in = startIncrement; in > 0; in = increment.next()) {
      ...
      for (int i = in; i < arrLen; i++) {
        int j = i;
        // 从分组尾部到头部，将
        while (j - in >= 0 && comparator.compare(data[j - in], data[j]) > 0) {
  ...
  public T[] sort(T[] data, Comparator<T> comparator) {
    ...
    return sort(data, comparator, new ShellIncrement(data.length));
  }
